...
- **`milestones/`** — your weekly project milestones. Each week you work through a milestone in your IDE, applying the skills from that week's tutorials.
...
Your weekly project milestones. Each milestone folder is self-contained: it includes the milestone instructions (`.qmd`), the starter scripts, a `data/` folder, and an `assets/` folder with the solution images referenced in the instructions.

| Folder | Files | Milestone |
|--------|-------|-----------|
| `milestones/shiny_01/` | `shiny_01.qmd`, `exercise1.R`, `exercise2.R`, `exercise3.R`, `data/`, `assets/` | Week 1 — Welcome to Shiny: arranging layouts with `layout_columns()`, tabbed UIs with `navset_card_tab()`, and `page_navbar()` |
| `milestones/shiny_02/` | `shiny_02.qmd`, `exercise1.R`, `exercise2.R`, `exercise3.R`, `data/`, `assets/` | Week 2 — Reactivity and modern UIs: downloads, `dateRangeInput()`, and file uploads with `fileInput()` |
| `milestones/shiny_03/` | `shiny_03.qmd`, `app.R`, `data/`, `assets/` | Week 3 — Theming and publishing: `bs_theme()` / `bs_themer()` / `thematic_shiny()`, then publishing to Posit Connect |
...
Open the milestone's `.qmd` file to read the instructions, then work through the exercises in the accompanying `.R` scripts. Run an app the same way as the tutorial files, using a path qualified to the milestone folder, e.g.
